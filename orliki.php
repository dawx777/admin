...
$row = $result->fetch_array(MYSQLI_ASSOC);
       ...
       echo "<h5>Organ Zarządzajcy orlikiem</h5><ul>";
    ...
    echo"Organ:"." ".$row['Nazwa_organu']."<br/>" ;
       ...
       $result=queryMysql("select login,termin.data,termin.czas_rozpoczecia,termin.czas_zakonczenia,orlik.nazwaorlika from administrator inner join rezerwacja on rezerwacja.Administrator_id=administrator.id inner join termin on termin.rezerwacja_id=rezerwacja.id right join orlik on termin.Orlik_id=orlik.id where nazwaorlika='$orlik' ");
